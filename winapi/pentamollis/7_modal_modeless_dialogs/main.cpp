...
LRESULT CALLBACK DialogProcedure(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_COMMAND:
        ...
        {
        case 1:
            // re-enable main window when model dialog is closed by Close button.
        ...
        break;
    case WM_CLOSE:
        // re-enable main window when model dialog is closed by X button.
        EnableWindow(hMainWindow, true); 
        DestroyWindow(hWnd);
        break;
    default:
...
void displayDialog(HWND hWnd)
{
    ...
    (
        ...
        L"Close",               // text caption
        WS_VISIBLE | WS_CHILD,  // style
        20,                     // x
        20,                     // y
        100,                    // width
        40,                     // height
        hDlg,                   // parent container
        (HMENU) 1,              // ID (need to cast)
        NULL,                   // hInstance (unused)
        NULL                    // lpParam (unused)
    );
    
    // Make it modal i.e. disable main window while dialog is open.
    EnableWindow
    (
        hWnd,   // window to enable/disable
